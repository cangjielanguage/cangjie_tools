...
```text
[profile.build]
lto = "full"  # 是否开启 `LTO` （Link Time Optimization 链接时优化）优化编译模式，该功能仅支持目标平台为 `Linux/OpenHarmony/Android/iOS` 平台, 效果同 `[profile.build.lto]` 的 `level` 配置项。后续该字符串类型配置项将被废除，请使用配置项 `[profile.build.lto]` 的 `level` 配置项开启LTO功能
...
- `lto` 指定是否开启 `LTO` 优化编译模式，该值可为 `thin` 或 `full` ，该功能仅支持目标平台为 `Linux/OpenHarmony/Android/iOS` 平台
